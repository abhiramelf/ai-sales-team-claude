...
Usage:
    ...
    python3 generate_pdf_report.py --config ~/.claude/sales-config.json data.json
...
Config support:
    --config <path>     Path to JSON config file (e.g. ~/.claude/sales-config.json).
                        Reads "company_name", "your_name", and "your_role" to use
                        in PDF header/footer branding.
    Falls back to default branding ("AI Sales Team Report") when no config is provided.
"""
...
def add_header_footer(canvas, doc, report_title="", branding=None):
    """Add header and footer to each page.

    Args:
        canvas: ReportLab canvas.
        doc: ReportLab document.
        report_title: Text shown in the header (typically the report date).
        branding: Optional dict with keys "header_label" and "footer_label" for
                  custom branding. Falls back to defaults when not provided.
    """
    if branding is None:
        branding = {}
    header_label = branding.get("header_label", "Sales Pipeline Report")
    footer_label = branding.get("footer_label", "Generated by AI Sales Team for Claude Code")
    ...
    canvas.drawString(54, letter[1] - 35, f"{header_label} — {report_title}")
    ...
    canvas.setFillColor(SECONDARY_TEXT)
    canvas.drawCentredString(letter[0] / 2, 28, footer_label)
    canvas.restoreState()


def generate_report(data, output_path, branding=None):
    """Generate the full PDF report from data dict.

    Args:
        data: Report data dictionary.
        output_path: Path for the output PDF file.
        branding: Optional dict with "header_label" and "footer_label" for
                  custom header/footer text. Falls back to defaults.
    """
    styles = build_styles()
    ...
    doc.build(
        story,
        onFirstPage=lambda c, d: add_header_footer(c, d, report_date, branding=branding),
        onLaterPages=lambda c, d: add_header_footer(c, d, report_date, branding=branding),
    )

    return output_path


def _build_branding(config_path):
    """Build branding dict from a config file, or return None for defaults."""
    cfg = load_config(config_path)
    if not cfg:
        return None
    company = cfg.get("company_name", "")
    name = cfg.get("your_name", "")
    role = cfg.get("your_role", "")

    # Build header label
    header_parts = ["Sales Pipeline Report"]
    if company:
        header_parts = [f"{company} — Sales Pipeline Report"]

    # Build footer label
    footer_parts = []
    if name and role:
        footer_parts.append(f"Prepared by {name}, {role}")
    elif name:
        footer_parts.append(f"Prepared by {name}")
    if not footer_parts:
        footer_parts.append("Generated by AI Sales Team for Claude Code")

    return {
        "header_label": header_parts[0],
        "footer_label": footer_parts[0],
    }


def main():
    parser = argparse.ArgumentParser(
        description="Sales Pipeline PDF Report Generator — AI Sales Team for Claude Code",
        epilog="Example: python3 generate_pdf_report.py data.json report.pdf",
    )
    parser.add_argument("input_file", nargs="?", default=None,
                        help="Path to input JSON data file (runs demo mode if omitted)")
    parser.add_argument("output_file", nargs="?", default=None,
                        help="Output PDF file path (default: SALES-REPORT.pdf)")
    parser.add_argument("--config", default=None,
                        help="Path to JSON config file (e.g. ~/.claude/sales-config.json) for custom branding")
    args = parser.parse_args()

    # Build branding from config if provided
    branding = _build_branding(args.config) if args.config else None
    ...
    if not args.input_file:
        ...
        output = args.output_file or "SALES-REPORT-sample.pdf"
        generate_report(sample_data, output, branding=branding)
        print(f"Sample report generated: {output}")
        return

    json_path = args.input_file
    output_path = args.output_file or "SALES-REPORT.pdf"

    with open(json_path, "r") as f:
        data = json.load(f)

    generate_report(data, output_path, branding=branding)
